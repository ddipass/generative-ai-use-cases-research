...
The argument should be the term, and the processing should be written recursively. Please output in markdown.`,
  ...
  const demoSummarize = () => {
    const params: SummarizePageQueryParams = {
      sentence:
        'Amazon Bedrock is a fully managed service that allows access to foundation models (FMs) provided by Amazon and major AI startup companies through an API. This enables you to choose from various FMs to find the model best suited for your use case. Amazon Bedrock's serverless experience allows you to quickly start with FMs, easily try them out, privately customize FMs using your own data, and seamlessly integrate and deploy FMs into your applications using AWS tools and features. Amazon Bedrock's agents are a fully managed capability that allows developers to easily create generative AI applications that can provide up-to-date answers based on their own knowledge sources and complete tasks for a wide range of use cases. Bedrock's serverless experience allows you to start using it immediately, privately customize FMs using your own data, and easily integrate and deploy them into your applications using familiar AWS tools and features (including integration with Amazon SageMaker's ML capabilities for experiments to test various models and pipelines to manage FMs at scale) without managing infrastructure.',
      additionalContext: '',
    };
